...
#define ADDRESS_FLAG_APP1           *(volatile unsigned int*) (0x08004000)
#define ADDRESS_FLAG_APP2           *(volatile unsigned int*) (0x08012000)

OTA_Header_t *pOTA_Info = (OTA_Header_t *)ADDRESS_FLAG_APP_CHECK; 
...
int main(void) 
{
    // With firt flash no have APP2
    // if(pOTA_Info->Magic_Word != 0X5A5A5A5A){
    //     Jump_To_App(ADDRESS_FLAG_APP1);
    // }

    // if (pOTA_Info->Active_App == 2) {
    ...
    if ((GPIOC_IDR & (1U << 13)) != 0) { 
        Jump_To_App(0x08012000); 
    } else {
        Jump_To_App(0x08004000); 
    }
